...
file_put_contents('b2c_result_log.txt', json_encode($_POST, JSON_PRETTY_PRINT) . PHP_EOL, FILE_APPEND);
...
try {
    ...
    // If transaction failed, refund the amount to wallet
    if ($result_code !== '0' && $affected_rows > 0) {
    ...
    // Log error
    file_put_contents('b2c_error_log.txt', $e->getMessage() . PHP_EOL, FILE_APPEND);
    // Respond to Safaricom
    header('Content-Type: application/json');
