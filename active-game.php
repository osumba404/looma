        ...
        if ($scramble_data) {
            ...
            if ($scramble_type === 'single') {
                ...
                if ($user_answer === $correct_word) {
                    $is_correct = true;
                    $win_amount = 50; // Reward for single word
                }
            } elseif ($scramble_type === 'multiple') {
                ...
                if ($user_words === $correct_words) {
                    $is_correct = true;
                    $win_amount = 100; // Reward for multiple words
                }
            } elseif ($scramble_type === 'anagrams') {
                ...
                if ($all_correct && count($user_words) === count($possible_words)) {
                    $is_correct = true;
                    $win_amount = 150; // Reward for anagrams
                }
            }
...
<head>
    ...
    <style>
        ...
        .spin-btn:hover, .scramble-btn:hover {
            background-color: var(--secondary-color);
        }
        .spin-btn:disabled, .scramble-btn:disabled {
            background: #666;
            cursor: not-allowed;
        }
        .bet-input, .scramble-input {
            margin: 1rem 0;
            padding: 0.5rem;
...
<body>
    ...
    <div class="main-content" id="mainContent">
        ...
        <section class="game-section py-4 animate-fadeIn">
            <div class="container">
                ...
                <!-- Spin & Earn Section -->
                <div id="spinGame" class="game-content">
                    <?php echo $result; ?>
                    <h3 class="mb-4">Spin & Earn</h3>
                ...
                <div id="scrambleGame" class="game-content" style="display: none;">
    ...
    <script>
        ...
        document.getElementById('gameSelect').addEventListener('change', function() {
            const spinGame = document.getElementById('spinGame');
            const scrambleGame = document.getElementById('scrambleGame');

            if (this.value === 'spin') {
                spinGame.style.display = 'block';
                scrambleGame.style.display = 'none';
            } else {
                spinGame.style.display = 'none';
